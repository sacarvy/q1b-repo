...
<template>
  <div
    class="dark:bg-slate-900 bg-slate-100 flex flex-col items-center min-h-screen pt-6 pb-8"
    @click="save"
  >
    <div
    ...
    >
      <ClientOnly>
        <Tiptap
          :is-colab="false"
          :user="{ name: user_id, color: user_color.hex }"
          :id="slug"
          :document-name="page.name"
          @click.stop="editMode"
          v-model:editing-value="page.editing"
          v-model="page.body"
        />
      </ClientOnly>
    </div>
  ...
  <div class="flex flex-col items-end fixed right-10 bottom-8 gap-y-4">
    ...
    >
      ...
      <Icon
        ...
        :name="
          page.saving ? 'heroicons:document-text' : 'heroicons:cloud-arrow-up'
        "
        class="w-5 h-5"
      />
    </button>
